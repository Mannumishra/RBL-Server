...
exports.CreateCardDetails = async (req, res) => {
    ...
    try {
        const { creditLimit, totalAmountDue, cardHolderName, cardNumber, expiryMonth, expiryYear, cvv } = req.body;
        ...
        // Total Amount Due validation (required, should be a positive number)
        ...
        } else if (typeof cardHolderName !== 'string' || cardHolderName.trim().length < 3) {
        ...
        const newCardDetails = new CardModel({ creditLimit, totalAmountDue, cardHolderName, cardNumber, expiryMonth, expiryYear, cvv });
